...
#### Очередь после разморозки 2026-08-23

До 23.08 действует запрет на деплой изменений схемы БД и логики сбора
(идёт замер прироста от пятой биржи). Накопленная очередь, по приоритету:

1. **`limit` в `OrderBookCollector`: 20 → 100.** Замер
   (`scripts/analysis/order_book_depth_probe.py`, 2026-08-18) показал, что
   20 уровней занижают глубину стакана в 2-26 раз. 100 — публичный потолок
   KuCoin (`level2_100`), внутри лимитов остальных бирж; покрывает $5000 в
   13 случаях из 14. Блокирует пункт 2.
2. **Пересчёт кривой «прибыль от размера сделки»** на данных с новой
   глубиной (`scripts/analysis/trade_size_curve.py`). Текущий результат —
   медианный оптимум $444 — получен на 20 уровнях и является НИЖНЕЙ
   границей. От него зависит рабочий размер сделки.
3. **Коллектор статуса переводов** — спроектирован (см. HANDOFF.md):
   ключ `(биржа, монета, сеть)`, сопоставление сетей по contract_address,
   5 bulk-запросов на цикл, схема по образцу funding rates. Закрывает
   слепое пятно: статус проверен лишь у 131 комбинации из 1121.
4. **Починка `target_volume` в `SpreadMonitor._calc_slippage`** — сейчас
   туда передаётся суточный объём вместо размера сделки. Чинить аккуратно:
   в нынешнем виде дефект даёт полезную метрику глубины стакана, её надо
   не потерять, а вынести отдельным полем.
5. **TimescaleDB: гипертаблицы + сжатие.** Триггер (пятая биржа) сработал.
   Проверено: мониторы не читают большие таблицы истории в рантайме,
   поэтому сжатие не влияет на генерацию сигналов. Требует перевода
   `timestamp` с epoch `DOUBLE PRECISION` на `TIMESTAMPTZ`. Предусловие
   для шестой биржи — по прогнозу она на текущий диск не влезает.
...
- [x] **Рост БД замедлился до ~195 МБ/сутки — закрыто наблюдение от
  ...
  - Не проверено окончательно, что это устоявшийся темп, а не разовое
    ...
    закрытым навсегда.
  - **ПЛАТО ПОДТВЕРЖДЕНО ФАКТОМ 2026-08-07, уже на retention=7** (запись
    выше сделана при retention=14). Замер через сутки после первого
    полного суточного цикла на новом горизонте:
    ```
                              06-08 22:48      07-08 21:02
    futures_spread_history    21 GB + 11 GB    21 GB + 11 GB
    spread_history          4326 MB + 3550 MB  4326 MB + 3550 MB
    arbitrage_opportunities 5182 MB +  379 MB  5182 MB +  393 MB
    БД целиком                    46 GB            46 GB
    диск /                    82%, 14 ГБ       82%, 14 ГБ
    ```
    Рост за сутки — 14 МБ индекса `arbitrage_opportunities`. Возраст
    данных 7.01 суток, ровно по retention. Прогноз «+6 ГБ за месяц» и
    оценка «450 МБ/сутки» были ВЕРХНИМИ и не подтвердились.
    **Почему это ожидаемо, а не удача:** место после `DELETE` не
    возвращается ОС, а переиспользуется под новые вставки — при
    установившемся окне retention файлы БД перестают расти вовсе. Это то
    же свойство, из-за которого 82% диска не упадут сами.
    **Заодно подтвердилось в тот же замер:** суточное расписание
    архиватора удержалось — прогон 06-08 20:52:01 → 07-08 20:52:08,
    дрейф 7 секунд (персистентный `_last_run`, 5.1 выше). Раньше каждый
    рестарт сбрасывал его в ноль.
  - **ПРОВЕРКА ЧЕРЕЗ 9 СУТОК АВТОНОМНОЙ РАБОТЫ (2026-08-15): БД не в
    плато, а УМЕНЬШИЛАСЬ.** Формулировка «плато» выше описывала верно
    механику, но не предсказала направление:
    ```
                              07-08 21:02      15-08 21:29
    futures_spread_history    21 GB heap       10155 MB heap   (41М строк)
    spread_history          4326 MB heap       1858 MB heap    (11М строк)
    arbitrage_opportunities 5182 MB heap       5182 MB heap
    БД целиком                    46 GB            33 GB
    диск /                    82%, 14 ГБ       66%, 24.5 ГБ
    ```
    Возраст данных прежний — 7.02 суток, retention работает штатно.
    `vacuum_count = 0`, ручного `VACUUM FULL` не было: сжатие дал
    autovacuum вместе со снизившимся притоком. То есть утверждение «место
    после DELETE не возвращается ОС» верно как правило, но не абсолютно —
    autovacuum способен отдать хвостовые страницы, и на горизонте недели
    это дало 13 ГБ.
  - **Приток снизился, это наблюдение, НЕ подтверждённая поломка:**
    `futures` 8.9М → 5.6М строк/сутки, `spread` 2.58М → 1.51М (сравнение
    06-08 и суток к 15-08 21:00). Провал 14-08 был глубже (3.37М), затем
    восстановился. Данные пишутся, paper trading идёт, все 8 бирж
    доступны. **Как это проверять дальше, чтобы не отмахнуться:** если
    списать снижение на «рынок» дважды подряд и тренд продолжит падать —
    прекратить объяснять рынком и копать системно (сверить число
    активных пар на бирже, долю отфильтрованных по свежести, длительность
    цикла). Объяснение по умолчанию не должно становиться постоянным.
  - **Инцидент 2026-08-15, для истории:** Gate.io Futures — 650 ошибок
    (таймауты) в окне 11:00-18:00 UTC, пик 12-14 ч, самовосстановление,
    к 20:54 все 8 бирж снова доступны. Четвёртый всплеск такого рода
    (были KuCoin 429, 503, 400001) — та же форма: короткий,
    самовосстанавливающийся, главный цикл не прерывался. Push-монитор
    намеренно НЕ сработал: отвал одной биржи из восьми не повод для
    алерта.
...
- [x] **CEX биржи: Gate.io и MEXC — добавлены** (API + Collector по шаблону раздела 4, 
  ...
  - **Уточнение по MEXC:** API (exchangeInfo) сообщает per-symbol комиссии 
    ...
    скидке — задача 5.3.
- [x] **Bybit — пятая биржа, добавлена 2026-08-16** (коммиты `ab48de7`
  API-клиент, `9e761aa` Collector'ы, `11e7ae0` интеграция). Slug
  `bybit`/`bybit_futures`, spot + linear-перпетуалы.
  **Три особенности Bybit, найденные разведкой и заложенные в архитектуру
  сразу, а не «оптимизацией потом»:**
  - `/v5/market/tickers` без параметра `symbol` отдаёт ВСЕ пары категории
    одним запросом (spot 556, linear 826). Цикл стоит один-два запроса
    вместо 1300+. Hummingbot тянет тикер по одному символу — этот его
    приём нам противопоказан.
  - `fundingRate`/`nextFundingTime` приходят прямо в тикерах linear:
    отдельного сетевого вызова под funding у Bybit нет вообще, в отличие
    от Binance Futures.
  - Единый REST V5 с параметром `category` (`spot`/`linear`) — один
    базовый класс закрывает и спот, и фьючерсы, не как у Binance с
    разными хостами.
  **Две грабли, закрытые в коде (обе проверены, не взяты из доков):**
  Bybit отвечает HTTP 200 при ошибке, признак — `retCode != 0` в теле
  (без проверки ошибка выглядела бы успехом, и в БД молча шли бы пустые
  списки); в категории `linear` живут квартальные `LinearFutures`
  (`BTCUSDT-04SEP26`) с ТЕМИ ЖЕ `baseCoin`/`quoteCoin`, что у бессрочного
  `BTCUSDT` — без фильтра по `contractType` их `standardized_pair`
  совпал бы с перпетуалом и дал ложные «возможности» (класс проблемы из
  раздела 6).
  Проверено: 27/27 живым прогоном против API, 42/42 сверкой симметрии
  интеграции, полный локальный цикл на дев-БД, деплой на прод 00:21:37
  UTC без единой ошибки. Время скана не выросло: 4.19–4.65 с при пяти
  биржах против ~4.0–4.5 с при четырёх.

  **МОДЕЛЬ РОСТА ОБЪЁМА — считать ДО добавления биржи, не после.**
  `futures_spread_monitor` генерирует комбинации автоматически, поэтому
  рост квадратичный: n_спот × n_фьюч + C(n_фьюч, 2).
  ```
    4 биржи → 16 + 6  = 22   (было)
    5 бирж  → 25 + 10 = 35   (Bybit, +59%)
    6 бирж  → 36 + 15 = 51   (+46%)
    7 бирж  → 49 + 21 = 70   (+37%)
    8 бирж  → 64 + 28 = 92   (+31%)
  ```
  Переход 4→5 подтверждён ТРИЖДЫ независимо (расчёт, локальный тест,
  прод — все дали 35). Но это воспроизводимость на одном шаге, а не
  предсказание: переход 5→6 проверит формулу вперёд, и это последняя
  дешёвая возможность её проверить до того, как упрёмся в диск.
  **Практический триггер:** при запасе ~15 ГБ после Bybit решение по
  диску становится ОБЯЗАТЕЛЬНЫМ примерно на 7-й бирже. Поэтому биржи
  добавляются строго ПО ОДНОЙ с замером между ними — иначе прирост от
  каждой не отделить.

  **BASELINE перед деплоем Bybit, снят 2026-08-16 00:20:24 UTC**
  (продублирован здесь намеренно: в `HANDOFF.md` он тоже есть, но тот
  файл в `.gitignore` и переживает сессию хуже):

  | таблица | heap, байт | index, байт | n_live_tup |
  |---|---|---|---|
  | `futures_spread_history` | 10 648 281 088 | 11 715 182 592 | 41 667 254 |
  | `arbitrage_opportunities` | 5 433 401 344 | 513 744 896 | 4 359 298 |
  | `spread_history` | 1 948 180 480 | 3 722 330 112 | 11 331 756 |
  | `funding_rate_history` | 202 235 904 | 269 082 624 | 2 387 427 |
  | `simulated_trades` | 384 901 120 | 17 637 376 | 226 928 |

  `pg_database_size` = 34 909 181 619 (32.5 ГБ); диск `/` свободно
  24.7 ГБ, занято 66%; комбинаций до деплоя — 22.
  **Замер прироста — 2026-08-23**, не раньше: полный эффект появится
  только когда окно retention (7 суток) целиком заполнится данными пятой
  биржи. Мерить раньше — занизить оценку.
  - **Кандидаты дальше:** OKX и Bitget (в Hummingbot 2.16.0 есть и spot,
    и perpetual-коннекторы — то есть будет с чем сверяться), затем
    Kraken. Дальний кандидат — Bitunix (Trust Score 9/10 на CoinGecko,
    но ликвидность тоньше топовых). Добавлять только после замера 23.08.
  - **Замечание из разведки Hummingbot:** у MEXC там ТОЛЬКО spot-коннектор,
    `mexc_perpetual` отсутствует — по MEXC Futures свериться с чужой
    реализацией не с чем, если возникнут вопросы.
...
- [x] **Веб-интерфейс — дашборд с графиками и метриками — реализовано 2026-07-21
  ...
  - Подключается к БД через новую read-only роль PostgreSQL
    ...
    компрометация Metabase не даёт возможности писать в прод-БД
  - **Порт 3000 ЗАКРЫТ 2026-08-07** (был `0.0.0.0:3000` + `ALLOW` в `ufw`
    — единственный порт кроме SSH, открытый наружу). Теперь биндинг
    `127.0.0.1:3000`, в `ufw` открыт только 22/tcp, доступ через
    SSH-туннель: `ssh -L 3000:localhost:3000 root@46.62.155.17`, затем
    `http://localhost:3000`. Пароль самого Metabase по-прежнему задаётся
    пользователем в веб-мастере и не хранится в коде/конфигах проекта.
    Причина закрытия и цифры — в записи 2026-08-07 ниже.
...
- [x] **Uptime Kuma — проактивный мониторинг главного цикла. СДЕЛАНО И
  ...
  **Про сам порог 10 ГБ — оценка УТОЧНЕНА 2026-08-07, прежняя была
  верхней и не подтвердилась.** Исходно считали ≈450 МБ/сутки по замеру
  03-08→06-08 и ждали срабатывания в середине месяца. Сутки наблюдения
  на retention=7 показали органический рост ≈ 0 (см. запись про плато в
  5.1): БД те же 46 ГБ, размеры таблиц байт в байт. Штатно порог не
  сработает.
  **НО свободное место всё равно сократилось — не из-за нашего бота:**
  13.5 ГБ → **11.69 ГБ**, потому что 2026-08-07 на тот же корневой раздел
  лёг swap-файл 2 ГБ, добавленный в рамках СОСЕДНЕГО проекта
  (копитрейдинг, `/root/copy-trading`). Запас до порога сжался с 3.5 ГБ
  до **1.69 ГБ**.
  **Порог решено НЕ менять** (2026-08-07): рост ≈ 0, значит запас не
  тратится сам, а понижение порога только сократило бы время на реакцию.
  **Что при этом важно помнить, если алерт всё же придёт:** за сутки
  06-08→07-08 общий VPS прибавил Kuma (2.48 ГБ) и swap (2 ГБ) — 4.5 ГБ, и
  ни один байт из них не принадлежит арбитражному боту. Следующее такое
  «маленькое» добавление выбьет порог, и сигнал придёт нам, хотя причина
  будет в соседнем проекте. Диск общий — это правильное поведение
  монитора, но не принимать его автоматически за проблему бота: сначала
  сверить `du -sh /var/lib/docker /swapfile /root/copy-trading`.
  Сосед на 2026-08-07 занимает мало: `/root/copy-trading` — 56 МБ, его
  SQLite — 12 МБ.
  **АКТУАЛЬНО НА 2026-08-15 — ТРЕВОГА РАССОСАЛАСЬ САМА, руками ничего не
  делали. Это прецедент, ради которого запись выше сохранена целиком:**
  ```
              07-08 (тревожно)      15-08 (спокойно)
  диск /      84%,  11.69 ГБ        66%,  24.47 ГБ
  БД          46 ГБ                 33 ГБ
  запас до порога 10 ГБ   1.69 ГБ           14.5 ГБ
  ```
  Причина — НЕ ручная чистка: `vacuum_count = 0` по всем таблицам,
  `VACUUM FULL` никто не запускал, индексы из «резерва 11 ГБ» на месте
  (8245 МБ и 2559 МБ, см. 5.5). Сработали autovacuum и снизившийся
  приток: heap `futures_spread_history` усох 21 → 10 ГБ, строк 53М → 41М.
  **Практический вывод на будущее:** увидев похожее сжатие запаса по
  диску, не бросаться удалять данные или дропать индексы — сначала дать
  системе несколько суток и посмотреть, не выправится ли само. Здесь
  паническое действие 07-08 (дроп индексов ради 11 ГБ) оказалось бы
  вредным: место вернулось без него, а индексы пришлось бы
  пересоздавать.
  **Побочно:** swap соседнего проекта, поставленный 07-08 «как страховка
  от неизвестного», на 15-08 реально используется — 838 МБ из 2 ГБ
  (07-08 было 0 Б). То есть память на VPS действительно подходила к
  краю, и решение соседа задним числом оправдалось.
- [x] **Порт 3000 (Metabase) закрыт 2026-08-07.** Пункт был вынесен из
  задачи Uptime Kuma отдельной строкой именно чтобы не уехать в закрытую
  задачу невыполненным — и это сработало: закрыт следующим же заходом.
  **Риск оказался не теоретическим.** За неделю в логах Metabase: **504
  неудачных попытки логина** и перебор эксплойтов (`POST
  /api/v1/user/repos`, `POST /api/mcp`, `GET /user/sign_up`,
  `/.well-known/security.txt`) с адресов массовых сканеров (`66.132.x.x`
  — Censys, AWS-диапазоны `3.129`/`18.218`). То есть сервер не выбирали
  целью — порт 3000 в интернете находят автоматически.
  **Успешных входов НЕ было** — в логах только внутренние задачи
  Metabase; следов компрометации не обнаружено.
  **Почему не ограничились сильным паролем:** у Metabase была pre-auth
  RCE (CVE-2023-38646), которая срабатывает ДО формы логина — пароль от
  такого класса уязвимостей не защищает. Образ `latest` собран за 3
  недели до этого и не обновлялся; на месяц без присмотра это открытая
  дверь, за которой некому следить.
  **Как закрыто:** биндинг `0.0.0.0:3000` → `127.0.0.1:3000` в
  `/root/metabase/docker-compose.yml` (бэкап рядом), контейнер
  пересоздан, `ufw delete allow 3000/tcp` (снялись v4 и v6). Доступ —
  SSH-туннель, как у Kuma; вариант «ограничить по IP» отклонён, потому
  что домашний IP динамический и правило пришлось бы править при каждой
  смене. Проверено снаружи: `:3000` и `:3001` недоступны; изнутри
  `/api/health` → `{"status":"ok"}`.
  **Попутно снята gotcha про ручной `docker network connect`** (жила в
  `HANDOFF.md`, раздел про прод): вторая сеть
  (`multi-exchange-arbitrage_default`, нужна для резолва хоста
  `arbitrage-db`) закреплена в compose как `external` — раньше она
  терялась при каждом пересоздании контейнера и её надо было подключать
  руками. Теперь ручного шага нет.
  **Проверять связь Metabase → БД так:** `docker exec metabase nc -z -w 4
  arbitrage-db 5432`. НЕ через `/dev/tcp` (в образе `/bin/sh` это
  busybox, где его нет — даёт ложное «недоступно») и не через
  `curl telnet://host:5432` (PostgreSQL не отвечает на telnet, curl
  просто таймаутит).
...
- [x] **Рычаг «фильтрация комбинаций» ПРОВЕРЕН НА ДАННЫХ И ОТКЛОНЁН
  2026-08-16.** Гипотеза была правдоподобная: при 8 биржах будет 92
  комбинации спот×фьюч и фьюч×фьюч, и казалось, что пары двух мелких
  бирж с тонкой ликвидностью — шум, который занимает место наравне с
  ценными. Проверено срезом 120 000 записей `futures_spread_history`
  строго после деплоя Bybit (все 35 комбинаций присутствуют):
  ```
  максимум   mexc x mexc_futures      6.11% объёма
  минимум    bybit x binance_futures  1.16% объёма
  топ-10 из 35 (29% комбинаций) → 44% объёма
  нижние 10   (29% комбинаций) → ~13% объёма
  ```
  **Распределение почти равномерное, разброс всего 5×** — «мусорных»
  комбинаций не существует. Убрав 10 комбинаций из 35, потеряли бы ~13%
  объёма и ровно столько же сигнала: обмен один к одному, без выигрыша.
  **Почему так, задним числом очевидно:** объём определяется числом
  ОБЩИХ ПАР между двумя биржами, а не «качеством» комбинации, а крупные
  биржи торгуют примерно одним набором ликвидных пар. Контрпример к
  интуиции: `gate_futures x bybit_futures` даёт 3.73% объёма — больше,
  чем `binance x binance_futures` (1.71%).
  **Остаются три рычага по диску:** дроп неиспользуемых индексов
  (−11 ГБ разово, запись выше), retention (линейно), TimescaleDB
  hypertables + сжатие (10–20×, единственный радикальный).
  - **Открытая гипотеза, НЕ проверена — фильтрация по ликвидности ПАР,
    а не комбинаций.** Это не то же самое: внутри каждой комбинации могут
    сидеть неликвидные пары, которые пишутся наравне с BTC/ETH.
    Группировка выше была по биржам и на этот вопрос не отвечает.
    Проверить при замере 23.08, вместе с основным приростом от Bybit —
    тогда будут полные 7 суток данных, а не 40 минут.
  - **Методическая заметка (ошибка поймана до выводов, не после).**
    Первый расчёт дал средний |basis| в 118 МИЛЛИОНОВ процентов —
    использовалось `avg` на распределении с тяжёлыми хвостами. Источник
    хвостов: `CATUSDT` стоит `1.67e-06` на споте KuCoin/MEXC и `868` на
    Binance Futures — разные активы под одним тикером. Правильная метрика
    здесь — медиана и перцентили; после исключения коллизий медианный
    базис 0.59–1.56% по всем комбинациям, то есть данные здоровы.
    **Побочно подтверждено на реальных данных: механизм
    `suspected_collision` работает** — все такие записи помечены флагом,
    доля 896 из 300 000 (0.3%).
