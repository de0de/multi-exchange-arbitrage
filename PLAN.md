...
- [ ] **Подготовка к первому VPS-прогону (приоритет выше переноса на VPS, целевой срок — до 2026-07-18):**
  Дизайн и объёмы — см. `QUESTIONS.md` и `DATA_SPECIFICATION.md` (созданы 2026-07-14).
  Каждый шаг тестируется локально отдельно, затем объединяются для VPS:
  1. [ ] `spread_history` — история спредов (агрегат по паре, порог 0.2% + 5-мин 
     снэпшоты всех многобиржевых пар) + retention-job (блокирующее условие VPS)
  2. [ ] `FuturesSpreadMonitor` — запись спот-фьюч/фьюч-фьюч спредов с embedded 
     funding-снимком и детекцией коллизий. Только детекция и запись, БЕЗ симуляции — 
     Фаза 2 paper trading проектируется позже на этих данных
  3. [ ] `funding_rate_history` — INSERT при изменении ставки (существующие 
     UPSERT-таблицы funding не меняются)
...
- [ ] **Withdrawal fee parser (комиссии на перевод монет):**
  ...
  - **Источник данных частично найден (2026-07-14), задача реализована не на 100%:** 
    ...
    кеш по паттерну Order Book (см. выше).
