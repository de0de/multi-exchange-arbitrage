...
- [x] **Архив истории (data lake) — экспорт перед retention-удалением — реализовано 2026-07-15:**
  ...
  - **⚠ ПРИОРИТЕТ — запланировать на ближайшие дни, не в неопределённый
    бэклог (2026-08-01):** `run_if_due()` — синхронная
    ...
    **Не экстренно (второй прогон чист, механизм пока не проявляется в
    обычном режиме), но и не в общий бэклог наравне с производительностью
    round-trip'ов** — механизм остаётся в коде: если retention когда-либо
    отключат/включат заново, изменят `retention_days`, или бот будет
    недоступен несколько дней подряд и перезапустится с большим бэклогом —
    тот же (или более долгий) паралич может повториться. Это напрямую
    касается целостности данных, на которых строится весь
    `QUESTIONS.md`-анализ. **Рекомендация:** обернуть
    `run_if_due()`/`_archive_table()` в `asyncio.to_thread()` — минимальная
    хирургическая правка, тот же масштаб, что фикс п.5 (коммит `1b8d27d`).
    **Тестировать так же строго:** реально смоделировать локально большой
    бэклог (например, временно уменьшить `retention_days` на копии БД так,
    чтобы `_archive_table()` пришлось обработать много строк) и подтвердить,
    что главный цикл больше не замирает — не полагаться на рассуждение
    "to_thread() должен помочь".
...
- **`spread_history`: 3 строки за 14 дней с `best_bid_exchange = best_ask_exchange` (одна и та же биржа на обеих сторонах).** Обнаружено 2026-08-01 при анализе Q-001 (маршрут `Gate.io → Gate.io`), исключено из того анализа как нерелевантное (не арбитражная ситуация), но причина не расследована. Гипотеза (не подтверждена): `best_bid`/`best_ask` в истории считаются независимо — `max(bid)` и `min(ask)` по одному и тому же списку цен биржи за цикл — если у одной биржи оказались одновременно самый высокий bid и самый низкий ask среди всех участников (например, из-за протухших/аномальных котировок), обе роли достанутся ей. Не срочно (3 случая за 14 суток, не влияет на выводы), но стоит разобраться при следующем заходе в `SpreadMonitor`/`_save_history` — не исключено, что это симптом чего-то, что стоит явно отфильтровать в коде, а не только в анализе постфактум.
