...
- [ ] **Перенос проекта на удалённый сервер (VPS)** для автономной круглосуточной работы. 
  ...
  **Решения (2026-07-15):** старт с ЧИСТОЙ БД — локальная история содержит 
  ...
  развёртывания), rclone (опционально), lnav. **Диск (уточнение 2026-07-16, 
  см. DATA_SPECIFICATION.md п.2):** прирост записи ~2–2.3 ГБ/сутки по SQLite-мерке; 
  PostgreSQL хранит те же строки в ~1.5–2 раза тяжелее (заголовки кортежей, WAL, 
  индексы) + образы Docker. При 80 ГБ диска — стартовать с retention_days=7 
  (данные не теряются: уходят в csv.gz-архив) и пересмотреть после замера 
  первых суток; 14-дневный retention без сжатия Timescale требует ~100+ ГБ.
