...
- [x] **Архив истории (data lake) — экспорт перед retention-удалением — реализовано 2026-07-15:**
  ...
  - **Gotcha (2026-07-30): `_last_run` живёт только в памяти процесса, не
    ...
    **Рекомендация (не начато):** персистентное хранение `_last_run` (файл
    рядом с БД или отдельная строка в Postgres) вместо in-memory — рестарт
    не должен влиять на суточное расписание архивации.
    **Альтернативный вариант решения — `pg_cron`** (расширение PostgreSQL,
    задания хранятся в `cron.job` внутри самой БД, не зависят от жизненного
    цикла Python-процесса бота — устраняет проблему `_last_run` по
    конструкции, не патчем). Ограничение: `pg_cron` выполняет только SQL
    внутри БД, не имеет доступа к файловой системе — не заменяет запись
    `.csv.gz`-архива на Volume (это должно остаться в Python-коде), максимум
    взял бы на себя часть логики удаления/расписания. Не проверено, доступен
    ли в образе `timescale/timescaledb:latest-pg17` (`SELECT * FROM
    pg_available_extensions WHERE name = 'pg_cron'` — не выполнено). Не
    приоритет, сравнить с уже существующим вариантом (файл/строка в БД)
    когда дойдёт очередь до этой задачи.
...
- [ ] **ПРИОРИТЕТ: OOM-kill процесса бота, 2026-08-03 20:31:32 UTC —
  ...
  - **Данные не пострадали, проверено, не предположено:** файл
    `futures_spread_history_2026-08-03.csv.gz` на Volume — единственный,
    полный (229 МБ), без огрызков/дублей. Значит крах случился ДО открытия
    gzip-файла, то есть на фазе `COUNT(*)` — обычный read-only запрос,
    который не буферизует данные на стороне клиента.
  - **Это ОСЛАБЛЯЕТ, а не подтверждает гипотезу про отложенный "Фикс 2"**
    (буферизация `SELECT *` без server-side курсора) — прямых улик, что
    причина именно в архиваторе, нет. Не приписывать этому компоненту
    причину без дальнейших фактов.
  ...
  - **Следующий шаг, не сделано:** профилирование памяти живого процесса
    (`tracemalloc`/`memory_profiler` или периодический снимок RSS по
    компонентам), сверка с моментами архивации и других тяжёлых операций
    по времени (совпадение по времени — не значит причина, см. выше про
    "Фикс 2"), проверка долгоживущих структур (открытые paper trading
    позиции, история health_monitor) на накопление.
...
- [ ] **Uptime Kuma — проактивный мониторинг главного цикла (найдено
  2026-08-04, не начато).** Push-based уведомления в Telegram при сбое —
  ...
  **При реализации, когда дойдёт очередь:**
  - Docker: `louislam/uptime-kuma:2` (не `:1`, устаревший тег)
  - Логин/пароль администратора — сразу при первом запуске, до открытия порта
  - Firewall: порт 3001 ограничить конкретным IP, не открывать всему интернету
    (заодно вспомнить сделать то же для уже открытого порта 3000 Metabase —
    давно откладывалось)
  - Push-запрос — добавить в `health_monitor.py`, URL хранить в `.env`
  - Отдельный Telegram-бот-токен, не переиспользовать токен копитрейдинга
  - Обязательно протестировать сам алерт перед тем как полагаться на него в проде
