...
- `futures_spread_history`: ~5.8 млн строк/сутки (~0.6 ГБ) — доля связок с
  |basis| ≥ 0.2% оказалась выше первоначальных ожиданий (~315 строк/цикл);
...
Суммарный прирост БД ~1.2–1.5 ГБ/сутки; при retention 14 дней ≈ 17–20 ГБ
только под БД. Для SQLite на VPS выполнимо ТОЛЬКО с retention+экспортом
(раздел 6); уточнить прирост по факту первых суток реального прогона.
Это же — выполнение условия из PLAN.md 5.5 («потребность в аналитических
запросах»): миграция PostgreSQL/TimescaleDB актуальна, выполняется
отдельным заходом, не под дедлайн.
...
Объём (замер 2026-07-15): ~1 442 сравнимые связки → снэпшоты ~0.42 млн/сутки
+ пороговые записи ~315 строк/цикл (~5.4 млн/сутки) — доля связок с
|basis| ≥ 0.2% оказалась заметно выше ожидания «basis обычно <0.5%».
