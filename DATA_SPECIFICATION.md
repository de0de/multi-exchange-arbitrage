...
Суммарный прирост БД ~2–2.3 ГБ/сутки (после расширения фьючерсов до
22 сравнений); при retention 14 дней ≈ 28–32 ГБ только под БД. Решение
пользователя (2026-07-15): миграция на PostgreSQL/TimescaleDB выполняется
ДО переноса на VPS — эти объёмы штатны для TimescaleDB (сжатие старых
чанков даёт 10–20×). До миграции SQLite работает с retention+экспортом
(раздел 6); уточнить прирост по факту первых суток реального прогона.
